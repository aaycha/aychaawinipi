...
public class ImageAiService {
    ...
    private byte[] generateWithApi(String prompt) throws Exception {
        // 1) Hugging Face si token configuré
    ...
    private static String getHuggingFaceToken() {
        ...
        try {
            Path p = Paths.get("hf_token.txt"); // ✅ Path.of -> Paths.get
            if (Files.exists(p)) {
                t = new String(Files.readAllBytes(p), StandardCharsets.UTF_8).trim();
    ...
    private byte[] tryHuggingFace(String prompt, String token) {
        try {
            String json = "{\"inputs\":\"" + escapeJson(safe(prompt)) + "\"}";
    ...
    private static String sanitizeBase(String baseName) {
        ...
        if (safeBase.trim().isEmpty()) safeBase = "event";
        return safeBase;
    }
    ...
    private static byte[] generateLocalPosterPng(String baseName, String titre, String description, String type, String lieu) throws Exception {
        ...
        String bigTitle = (titre == null || titre.trim().isEmpty()) ? "Événement" : titre.trim();
        ...
        String typeStr = (type == null) ? "" : type.trim().toUpperCase();
        if (!typeStr.isEmpty()) {
    ...
    private static List<String> wrapText(String text, Font font, int maxWidth) {
        if (text == null) text = "";
        text = text.trim();
        if (text.isEmpty()) return Collections.singletonList("(no details)");
        ...
        for (String w : words) {
            String next = current.length() == 0 ? w : (current + " " + w);
