...
public class SmsService {
    ...
    /**
     * Primary method to send SMS. Tries Vonage first, fallbacks to TopMessage.
     */
    public boolean sendSMS(String to, String message) {
        // 1. Try Vonage (Primary)
        if (sendViaVonage(to, message)) {
            return true;
        }

        // 2. Fallback to TopMessage
    ...
    private boolean sendViaTopMessage(String to, String message) {
        ...
        try {
            ...
            String jsonPayload = String.format(
                    "{\"data\": {\"to\": \"%s\", \"from\": \"%s\", \"text\": \"%s\"}}",
                    to, sender, message.replace("\"", "\\\""));
            HttpRequest request = HttpRequest.newBuilder()
