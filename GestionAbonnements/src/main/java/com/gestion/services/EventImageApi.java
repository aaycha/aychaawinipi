...
public class EventImageApi {
    ...
    private static String buildShortPrompt(String titre, String desc, String type, String lieu) {
        ...
        if (!safe(titre).isBlank())
            sb.append("Subject: ").append(safe(titre)).append(". ");
        if (!scene.isBlank())
            sb.append("Scene: ").append(scene).append(". ");
        if (!safe(lieu).isBlank())
            sb.append("Location: ").append(safe(lieu)).append(". ");
    ...
    // ✅ CORRIGÉ (Java 11) : switch classique
    private static String typeTheme(String typeUpper) {
        switch (typeUpper) {
            case "SOIREE":
                return "night party, neon lights, music";
            case "RANDONNEE":
                return "mountains hiking, nature adventure";
            case "CAMPING":
                return "campfire, tent, forest, stars";
            case "SEJOUR":
                return "travel, vacation, scenic destination";
            default:
                return "celebration, gathering";
        }
    }
    ...
    private ImageAiService.GeneratedImage fallbackUnsplashOrDefault(
            String titre, String desc, String type, String lieu) throws Exception {
        ...
        // ✅ Fallback vers ressources si uploads vide
        try (InputStream is = getClass().getResourceAsStream("/images/logo.png")) {
            if (is != null) {
                byte[] bytes = readAll(is);
                return new ImageAiService.GeneratedImage(fallbackName, null, "resource-default", bytes);
            }
        }
