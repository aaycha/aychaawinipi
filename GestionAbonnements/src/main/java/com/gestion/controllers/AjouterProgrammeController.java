...
public class AjouterProgrammeController implements DataReceiver<Integer> {
    ...
    @FXML
    private Spinner<Integer> spDebutH;
    @FXML
    private Spinner<Integer> spDebutM;

    @FXML
    private DatePicker dpFin;
    @FXML
    private Spinner<Integer> spFinH;
    @FXML
    private Spinner<Integer> spFinM;
    ...
    private void chargerInfosEvenement() {
        ...
        Evenement e;
        try {
            e = evenementService.getOneById(idEvent);
        } catch (Exception ex) {
            ex.printStackTrace();
            lblEventInfo.setText("⚠️ Erreur chargement événement");
            return;
        }
    ...
    private void onAjouterProgramme(ActionEvent event) {
        ...
        Integer dh = spDebutH.getValue();
        Integer dm = spDebutM.getValue();
        Integer fh = spFinH.getValue();
        Integer fm = spFinM.getValue();

        if (dh == null || dm == null || fh == null || fm == null) {
            showError("❌ Heures invalides.");
            return;
        }

        LocalDateTime debut = LocalDateTime.of(dpDebut.getValue(), LocalTime.of(dh, dm));
        LocalDateTime fin = LocalDateTime.of(dpFin.getValue(), LocalTime.of(fh, fm));
    ...
    private void onRetour(ActionEvent event) {
        SceneUtil.switchToWithData("/Feryel/DetailsEvenement.fxml", "Détails Événement", idEvent);
    }
